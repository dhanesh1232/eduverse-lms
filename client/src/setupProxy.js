...
module.exports = function (app) {
  ...
  try {
    ...
    console.log(
      "⚠️ Could not read port-config.json, trying environment variables..."
    );
  }
  ...
  if (!backendPort) {
    backendPort =
      process.env.REACT_APP_BACKEND_PORT || process.env.BACKEND_PORT;
    if (backendPort) {
      portSource = process.env.REACT_APP_BACKEND_PORT
  ...
  if (!backendPort) {
    backendPort = "4000"; // Updated fallback to match new default
    portSource = "fallback";
  }
